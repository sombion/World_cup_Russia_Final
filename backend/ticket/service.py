...
async def buy_ticket(lottery_id: int, money: int, user_id: int):
    ...
    if count_ticket >= lottery_data.max_count_ticket:
        raise MaxUsersInLotteryException
    ...
    # Присмоение пользувателю номер в бд
    ticket_data: Ticket = await TicketDAO.buy(user_id, lottery_id)
    # Списание денег
